...
class HorizonNet(nn.Module):
    ...
    def __init__(self, backbone, use_rnn , max_predict_count = 10):
        ...
        self.v_head = nn.Linear(1024,4)
        self.du_head = nn.Linear(1024,1)
        self.cls_head = nn.Linear(1024,1)
        #self.fix_head.bias.data.normal_(0.5,0.5)
        ...
        if self.use_rnn:
            ...
            self.linear = nn.Linear(in_features=2 * self.rnn_hidden_size,
                                    ...
                                    out_features= self.step_cols * max_predict_count)
            #self.linear.bias.data[0*self.step_cols:1*self.step_cols].normal_()
            self.linear.bias.data.normal_(0.5,0.5)            
            '''
            ...
            self.linear = nn.Sequential(
                ...
                nn.Linear(self.rnn_hidden_size,  self.step_cols * max_predict_count),
            )
            '''
    ...
    def forward(self, x):
        ...
        if self.use_rnn:
            ...
            output = self.linear(output)  # [seq_len, b, step_cols * max_predict_count]
            output = output.view(output.shape[0], output.shape[1], self.max_predict_count , self.step_cols)  # [seq_len, b, max_count , 3, step_cols]
            output = output.permute(1 , 2 , 0, 3)  # [b, max_count , seq_len, step_cols]
            output = output.contiguous().view(output.shape[0] , self.max_predict_count , -1)  # [b, max_count, seq_len*step_cols]

            #output = torch.transpose(output , 1,2)            
            cls = self.cls_head(output)
            du = self.du_head(output)
            v = self.v_head(output)
        else:
            # Not Fixed
            feature = feature.permute(0, 2, 1)  # [b, w, c*h]
            output = self.linear(feature)  # [b, w, 3 * step_cols]
            output = output.view(output.shape[0], output.shape[1], 6, self.step_cols)  # [b, w, 3, step_cols]
            output = output.permute(0, 2, 1, 3)  # [b, 3, w, step_cols]
            output = output.contiguous().view(output.shape[0], 6, -1)  # [b, 3, w*step_cols]

        # output.shape => B x 3 x W
        #cor = output[:, :1]  # B x 1 x W
        #bon = output[:, 1:]  # B x 2 x W
        #print("output.shape",output.shape)   # e.g [2, 20, 5, 1024]
        '''
        u = output[:, :, :1]
        v_top = output[:, :, 1:2]
        v_btm = output[:, :, 2:3]
        ...
        #return bon, cor
        return u,v_top , v_btm ,d_top , d_btm
        '''
        #return output
        return torch.cat((cls , v[:,:,:2] , du , v[:,:,2:] ), 2)
