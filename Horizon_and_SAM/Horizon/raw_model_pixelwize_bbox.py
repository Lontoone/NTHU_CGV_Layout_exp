...
class HorizonNet(nn.Module):
    ...
    def __init__(self, backbone, use_rnn):
        ...
        if self.use_rnn:
            ...
            self.linear = nn.Linear(in_features=2 * self.rnn_hidden_size,
                                    ...
                                    out_features=6 * self.step_cols)
            self.linear.bias.data[0*self.step_cols:1*self.step_cols].fill_(-1)
            self.linear.bias.data[1*self.step_cols:2*self.step_cols].fill_(0.225)
            self.linear.bias.data[4*self.step_cols:5*self.step_cols].fill_(0.725)
        else:
    ...
    def forward(self, x):
        ...
        if self.use_rnn:
            ...
            output = output.view(output.shape[0], output.shape[1], 6, self.step_cols)  # [seq_len, b, 3, step_cols]
            output = output.permute(1, 2, 0, 3)  # [b, 3, seq_len, step_cols]
            output = output.contiguous().view(output.shape[0], 6, -1)  # [b, 3, seq_len*step_cols]
        else:
        ...
        prob = output[:, :1]  # B x 1 x W
        boxs = output[:, 1:]  # B x 2 x W
